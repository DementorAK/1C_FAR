...
fn is_protected_module(data: &[u8]) -> bool {
    ...
    if let Ok(s) = std::str::from_utf8(data) {
        let t = s.trim_start();
        if t.starts_with("//")
            || t.starts_with("╨Я╤А╨╛╤Ж╨╡╨┤╤Г╤А╨░")
            || t.starts_with("╨д╤Г╨╜╨║╤Ж╨╕╤П")
...
/// For objects embedded in CF, the path may differ тАФ try multiple.
...
fn build_cf_object_vfs(rows_map: &HashMap<String, Vec<u8>>, obj_uuid: &str) -> Vec<VfsEntry> {
    ...
    if let Ok(obj_parser) = parse_object(rows_map, obj_uuid) {
        // Object module: from [1][3][1][1][2] тЖТ {uuid}.0
...
///
/// CFE stores objects flat тАФ each object is a separate row with its own
