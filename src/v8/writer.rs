...
impl ContainerWriter {
    pub fn new(page_size: u32, is_64bit: bool) -> Self {
        Self {
            ...
            revision: 0,
            use_triplets: false,
            pad_pt_to_page: false,
        }
    }

    pub fn write<W: Write, F: FnMut(usize, usize)>(
        &self,
        writer: &mut W,
        rows: &HashMap<String, (Vec<u8>, bool)>,
        ...
        docs.sort_by(|a, b| {
            ...
            let id_a_u16: Vec<u16> = a.0[20..]
                .chunks_exact(2)
                .map(|c| u16::from_le_bytes([c[0], c[1]]))
                .collect();
            let id_b_u16: Vec<u16> = b.0[20..]
                .chunks_exact(2)
                .map(|c| u16::from_le_bytes([c[0], c[1]]))
                .collect();
        ...
        let end_marker = if self.is_64bit {
            u64::MAX
        } else {
            crate::v8::container::SIG as u64
        };
        ...
        for (h, b) in &docs {
            ...
            let b_pages = self.serialize_document_into_pages(
                b,
                &mut b_offset,
                end_marker,
                Some(self.page_size),
        ...
        for p in pointers {
            if self.is_64bit {
                pointers_raw.extend_from_slice(&p.to_le_bytes());
            } else {
                pointers_raw.extend_from_slice(&(p as u32).to_le_bytes());
            }
        }
        let mut pt_offset = img_header_size;
        let pt_force_size = if self.pad_pt_to_page {
            Some(self.page_size)
        } else {
            None
        };
        // Pointers table should force full page_size alignment only for root containers.
        let pt_pages = self.serialize_document_into_pages(
            &pointers_raw,
            &mut pt_offset,
            end_marker,
            pt_force_size,
    ...
    fn serialize_document_into_pages(
        &self,
        content: &[u8],
        current_offset: &mut u32,
        end_marker: u64,
        force_page_size: Option<u32>,
    ) -> Vec<Vec<u8>> {
    ...
    fn format_page_header(&self, full_size: u64, page_size: u64, next_page: u64) -> Vec<u8> {
        if self.is_64bit {
            format!(
                "\r\n{:016x} {:016x} {:016x} \r\n",
                full_size, page_size, next_page
            )
            .into_bytes()
        } else {
            format!(
                "\r\n{:08x} {:08x} {:08x} \r\n",
                full_size as u32, page_size as u32, next_page as u32
            )
            .into_bytes()
        }
    }
