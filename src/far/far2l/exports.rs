...
pub unsafe extern "system" fn SetStartupInfoW(info: *const PluginStartupInfo) {
    let _ = std::panic::catch_unwind(|| {
        if !info.is_null() {
            ...
            let root_key_ptr = (*info).RootKey;

            if !module_name_ptr.is_null() {
                let _module_name = crate::far::string_utils::from_wide_ptr(module_name_ptr);
                ...
                let ini_path = plugin_config_dir
                    .join("config.ini")
                    .to_string_lossy()
                    .to_string();
            ...
            info!("1C:Enterprise Artifacts plugin loaded (far2l)");
        }
    });
}
            ...
            if dir_str == ".." || dir_str == "\\" || dir_str == "/" || dir_str.is_empty() {
                if let Some(api) = crate::far::STARTUP_INFO {
                    if let Some(control) = api.Control {
                        control(h_plugin, crate::far::far2l::api::FCTL_CLOSEPLUGIN, 0, 0);
                    }
                }
            }
            0
        ...
        if event == 3 && panel.is_modified {
            ...
            if let Some(api) = crate::far::STARTUP_INFO {
                if let Some(message_fn) = api.Message {
                    let res = message_fn(
                        api.ModuleNumber,
                        crate::far::far2l::api::FMSG_WARNING,
                        ptr::null(),
                        items.as_ptr(),
                        items.len() as i32,
                        3,
